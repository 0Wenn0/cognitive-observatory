...
export default function HeroSection() {
  ...
  // DETECCIÓN DE MOBILE — JavaScript directo.
  // Solución al problema de especificidad CSS vs estilos inline:
  // los estilos inline siempre ganan sobre media queries CSS.
  // Con isMobile en el estado de React, los estilos se aplican
  // directamente en el JSX — sin conflictos de especificidad.
  ...
  const getMicroStyle = (position: string) => {
    ...
    if (position === 'bottom-left') return { ...base, bottom: '120px', left: isMobile ? '16px' : 'clamp(24px, 5vw, 64px)' };
    if (position === 'center')      return { ...base, top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center' as const };
    if (position === 'top-right')   return { ...base, top: '80px', right: isMobile ? '16px' : 'clamp(24px, 5vw, 64px)', textAlign: 'right' as const };
    return { ...base, bottom: '120px', left: isMobile ? '16px' : 'clamp(24px, 5vw, 64px)' };
  };
  ...
  // Botones reutilizables — se renderizan en dos lugares:
  // dentro del hero-statement en mobile (fluyen con el texto)
  // y en posición fija en desktop.
  ...
  return (
    <section style={{
      ...
      {/* HERO STATEMENT — bloques como latidos
          ...
          MOBILE: isMobile controla todos los valores directamente
          desde JavaScript — sin conflictos de especificidad CSS.
          Rayner (1998): texto en tercio izquierdo del viewport. */}
      {showHero && (
        <div
          style={{
            position: 'absolute',
            top: isMobile ? '52px' : '18vh',
        ...
        >
          ...
          <div style={blockStyle(block >= 2)}>
            <p style={{
              fontFamily: "'DM Serif Display', serif",
              fontSize: isMobile ? '18px' : 'clamp(18px, 2vw, 26px)',
          ...
          {/* Bloque 3 — Coda EN
              Kroll & Bialystok (2013): L2 activa red de
              posicionamiento estratégico global. */}
          <div style={blockStyle(block >= 3)}>
            <p style={{
              fontFamily: "'IBM Plex Sans', sans-serif",
              fontSize: isMobile ? '13px' : 'clamp(12px, 1.2vw, 15px)',
          ...
          {/* BOTONES MOBILE — solo en mobile, dentro del flujo.
              margin-top: auto los empuja al fondo del contenedor.
              padding-bottom reserva espacio para los indicadores.
      ...
      {/* BOTONES DESKTOP — posición fija, solo en desktop.
          bottom 48px · left 64px — idéntico al primer commit.
          Ocultos en mobile — los botones mobile están arriba
          dentro del hero-statement.
          Cialdini (2001): CTA como puerta abierta. */}
      <AnimatePresence>
        {showButtons && !isMobile && (
          <motion.div
